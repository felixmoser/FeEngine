...
class FeSoundElement {
	...
	protected FeSoundElement(final Context context, final int resourceId, final SoundLoadCallback callback) {
	...
	public void playSound(final float leftVolume, final float rightVolume, final boolean loop) {
		mStreamId = mSoundPool.play(mSoundId, leftVolume * mMasterVolume, rightVolume * mMasterVolume, 0, loop ? -1 : 0, 1f);
	...
	public void setChannelVolumes(final float leftVolume, final float rightVolume) {
	...
	private void updateVolume() {
		mSoundPool.setVolume(mStreamId, mLeftVolume * mMasterVolume, mRightVolume * mMasterVolume);
	}
}
