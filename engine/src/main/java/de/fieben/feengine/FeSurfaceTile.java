...
/**
 * This class delivers the content for {@link FeSurfaceMap} and represents a single cell in its grid.
 * It doesn't hold position parameters, they get's delivered as parameter in {@link #onDraw(Canvas, int, int, Paint)}.
 * 
...
public abstract class FeSurfaceTile {
	...
	protected final void draw(final Canvas canvas, final int x, final int y, final Paint paint) {
		if (mBitmapKey != -1) {
	 ...
	 * @param x The x-coordinate to draw this tile on.
	 * @param y The y-coordinate to draw this tile on.
	 * @param paint The {@link Paint} used for drawing.
	 */
	public abstract void onDraw(final Canvas canvas, final int x, final int y, final Paint paint);
}