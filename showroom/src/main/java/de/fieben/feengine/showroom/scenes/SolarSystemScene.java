...
public class SolarSystemScene extends FeSurface {
	...
	public SolarSystemScene(final Context context, final AttributeSet attrs) {
		...
		final AnimationTestElement mAnimationSun = new AnimationTestElement(bitmapKey, 4, 100);
		mAnimationSun.setTranslate(450, 450);
		addElement(mAnimationSun);

		// Mercury
		addSatellite(mAnimationSun, 5, 0.4f, 4, Color.GRAY);

		// Venus
		addSatellite(mAnimationSun, 9, 0.7f, 7, Color.YELLOW);

		// Earth
		final CircleElement earth = addSatellite(mAnimationSun, 10, 1f, 9, Color.BLUE);
		// Earth Moon
		addSatellite(earth, 2, 0.03f, 2, Color.LTGRAY);

		// Mars
		addSatellite(mAnimationSun, 7, 1.5f, 15, Color.RED);

		// Jupiter
		final CircleElement jupiter = addSatellite(mAnimationSun, 40, 2.5f, 30, Color.GREEN);
		...
		// Uranus
		addSatellite(mAnimationSun, 20, 5.2f, 120, Color.MAGENTA);

		// Neptune
		addSatellite(mAnimationSun, 20, 8.5f, 150, Color.CYAN);
	}
